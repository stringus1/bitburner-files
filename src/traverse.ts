...
const TRAVERSE_URL = 'https://raw.githubusercontent.com/stringus1/bitburner-files/master/traverse.js'
const TRAVERSE_RAM = 5.9

const PAYLOAD_NAME = 'hacker.js'
const PAYLOAD_URL = 'https://raw.githubusercontent.com/stringus1/bitburner-files/master/hacker.js'
const PAYLOAD_RAM = 3.25

const PORTS = ['NUKE.exe', 'BruteSSH.exe', 'FTPCrack.exe']

/** @param {NS} ns */
export async function main(ns: NS): Promise<any> {
	const currentHost = ns.getHostname()
	const blacklist = ns.args.map(elem => elem.toString()).concat(currentHost)
	const serverList = (await ns.scan()).filter(host => !blacklist.includes(host))
